...
jQuery
		.extend({
			...
			ajaxFileUpload : function(s) {
				...
				var extention = fielName.substring(
						fielName.lastIndexOf(".") + 1).toLowerCase();
				...
				if (s.global && !jQuery.active++) {
					jQuery.event.trigger("ajaxStart"); // 触发 AJAX
														// 请求开始时执行函数。Ajax 事件。
				}
				...
				var xml = {};
				if (s.global)
					jQuery.event.trigger("ajaxSend", [ xml, s ]); // 触发 AJAX
																	// 请求发送前事件
				// 上载完成的回调函数
				var uploadCallback = function(isTimeout) {
					...
					try {
						...
						} else if (io.contentDocument) { // contentDocument
															// Firefox 支持，> ie8
															// 的ie支持。可取得子窗口的
															// document 对象。
